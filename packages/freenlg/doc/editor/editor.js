...
function spawnEditor(lang, template) {
  ...
  if (toComplete[lang]==null) { toComplete[lang]=[] };
...
window.onload = function() {
  ...
  for (let i=0; i<langs.length; i++) {
    ...
    fileref.setAttribute("src", `freenlg_tiny_${langs[i]}_0.15.6_comp.js`);
...
function fillEditor(id, lang, template) {
  ...
  new Vue({
    ...
    methods: {
      ...
      compileRender() {
        let freenlg_module;
        if (this.language=='en_US') { freenlg_module = freenlg_en_US; }
        if (this.language=='fr_FR') { freenlg_module = freenlg_fr_FR; }
        if (this.language=='de_DE') { freenlg_module = freenlg_de_DE; }
        try {
          let rendered = freenlg_module.render(this.code, {
