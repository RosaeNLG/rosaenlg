...
* <<performance_integration.adoc#, Performance tuning, how to integrate, how to package to run in a browser, how to contribute>>.
* <<changelog.adoc#, Changelog / release note>>.
* If you wonder: <<what_is_nlg>> and <<usecases>>.

Also see <<detailed_characteristics>>.

You can play around with a client side editor, compiler and renderer: http://freenlg.org/demo/demo.html[in browser editor].
...
<script>
spawnEditor('en_US', 
`- var data = ['apples', 'bananas', 'apricots'];
test
  eachz fruit in data with { separator: ',', last_separator: 'and' }
    | #{fruit}
`
...
See <<filter.adoc, Filter mixins and functions>>.
...
anchor:detailed_characteristics[Detailed characteristics of FreeNLG]
...
FreeNLG has the ability to generate Yseop templates. Yseop is a NLG software vendor.
This feature should be used to migrate existing FreeNLG templates to Yseop.
See <<yseop.adoc#, Yseop template generator documentation>>.
