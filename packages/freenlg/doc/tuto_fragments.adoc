...
TIP: This tutorial focuses on server-side rendering using node.js. But you can run your templates client side in browser, see <<performance_integration#inbrowser>>.
...
* choosing a language (here `language: '{freenlg_lang}'`) is mandatory. 
...
WARNING: Here we have put the main loop directly in the Pug template. In real cases, it is better to loop outside (directly in the javascript caller), as this allows an easy reset of FreeNLG and Pug between each rendering, which is much better for performance.
