...
* `det` to add a determinant; current determinants are `DEFINITE`, `INDEFINITE` and `DEMONSTRATIVE`. In English, for `DEMONSTRATIVE`, use `dist` with `NEAR` (default) for this/these and `FAR` for that/those.
...
WARNING: This feature is still quite experimental. It is useful in French and in German mostly (less in English).
