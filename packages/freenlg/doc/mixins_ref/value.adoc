...
* `owner` to indicate an owner. See <<mixins_ref.adoc#thirdPossession>>.
...
WARNING: This feature is still quite experimental. It is mostly useful in French and in German (less in English).
...
When it is a *date*, the `moment` lib will be used to format it. You should put a parameter to indicate how to format it: `#[+value( new Date('1980-04-14') , {dateFormat:'LL'})]` will output _April 14, 1980_ in English.
