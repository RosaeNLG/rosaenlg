...
In French and Italian, default position of the adjective is after the noun: _les gâteaux délicieux_, but not always: _les bons gâteaux_. Set `adjPos` to `BEFORE` or `AFTER` to change the position :
++++
<script>
spawnEditor('fr_FR', 
`p
