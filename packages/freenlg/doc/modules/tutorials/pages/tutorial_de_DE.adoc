...
_Ich liebe das neue OnePlus 5. Der OnePlus 5 hat einen Akku von 3 300 mAh. Das Handy verfügt über ein Display mit einem Bildschirm-zu-Körper-Verhältnis von 72,93% und einer physischen Größe von 5,5 Zoll. Die verfügbaren Töne des Telefons sind Gold und Grau._ +
...
. We have not been able to replace `ich liebe das neue #{phone.name}`, because the article (`das`) is already present before the `neue` adjective. It's not a big issue, but this may issue in these texts: _Ich liebe das neue OnePlus 5. Der OnePlus 5 hat einen Akku von 3 300 mAh._
...
TIP: The empty line containing only a pipe `|` forces the insertion of a space. If you don't put it, you will have `derOnePlus`. It is difficult to anticipate those cases - simply add a `|` in an empty line when it happens.
...
FreeNLG will automatically choose the right article.
