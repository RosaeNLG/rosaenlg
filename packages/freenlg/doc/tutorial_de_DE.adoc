...
. We have not been able to replace `ich liebe das neue #{phone.name}`, because the article (`den`) is already present before the `neuen` adjective. It's not a big issue, but this may issue in these texts: _Ich liebe das neue OnePlus 5. Der OnePlus 5 hat einen Akku von 3 300 mAh._
...
Let's generate automatically the determinant. diese/dieses are demonstrative pronous, das/die are definite articles:
....
    syn
...
It can be tedious to split nominal groups in determinant, adjective and noun. Use the simplified syntax with `<...>`:
....
    syn
      if !hasFlag(params, 'NOT_ES')
        | #{getMorF(['er', 'sie', 'es'], phone)}
