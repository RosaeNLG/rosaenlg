...
type GenderNumberIndex = 'MS' | 'MP' | 'FS' | 'FP';
...
export function getDet(detType: DetType, gender: Genders, number: Numbers, dist: Dist): string {
  ...
  switch (detType) {
    case 'DEMONSTRATIVE':
      ...
      const dem: { [index: string]: GenderNumber } = {
        NEAR: { MS: 'questo', MP: 'questi', FS: 'questa', FP: 'queste' },
        FAR: { MS: 'quello', MP: 'quelli', FS: 'quella', FP: 'quelle' },
      };
      return (dem[dist] as GenderNumber)[(gender + number) as GenderNumberIndex] as string;

    case 'DEFINITE':
    case 'INDEFINITE':
      const dets: { [index: string]: GenderNumber } = {
        DEFINITE: { MS: 'il', MP: 'i', FS: 'la', FP: 'le' },
        INDEFINITE: { MS: 'un', FS: 'una' },
      };
      return (dets[detType] as GenderNumber)[(gender + number) as GenderNumberIndex] as string;
  }
}
