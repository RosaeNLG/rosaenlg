...
function load(): void {
  ...
  if (adjectives!=null) {
    ...
    //console.log('LOAD');
    adjectives = JSON.parse(fs.readFileSync(__dirname + '/../resources_pub/de_DE/adjectives.json', 'utf8'));
  }
}

export function agreeGermanAdjective(adjective: string, germanCase: string, gender: string, number: string, det: string): string {
  load();
