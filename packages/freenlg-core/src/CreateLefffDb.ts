...
const lefffpath:string = 'resources_src/fr_FR/lefff-3.4.mlex/lefff-3.4.mlex';

let db = new sqlite3('./resources_pub/fr_FR/lefff.db');

db.exec('DROP TABLE IF EXISTS lefff')
  .exec(`CREATE TABLE lefff(
      ff TEXT, 
      nature TEXT, 
      racine TEXT, 
      codes TEXT, 
...
try {
  lineReader.on('line', function (line) {
    ...
    if (codes.indexOf('m')>-1) { masc=1; }
    if (codes.indexOf('f')>-1) { fem=1; }
    if (fem==0 && masc==0) {
      fem = 1;
      masc = 1;
    }

    if (codes.indexOf('s')>-1) { sing=1; }
    if (codes.indexOf('p')>-1) { plu=1; }
    if (sing==0 && plu==0) {
      sing = 1;
      plu = 1;
    }
