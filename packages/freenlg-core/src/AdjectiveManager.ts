...
export class AdjectiveManager {
  ...
  getAgreeAdj_fr_FR(adjective: string, gender: string, number: string): string {
    let withGender: string = gender=='F' ? this.getAdjFeminine_fr_FR(adjective) : adjective;
    let withNumber: string = number=='P' ? this.getAdjPlural_fr_FR(withGender) : withGender;
    return withNumber;
  }
  ...
  Actuellement implémenté via des règles et des listes d'exceptions. Les règles sont nombreuses avec plein d'exceptions. L'implémentation faite actuellement a une bonne couverture, mais quelques impasses, et certainement quelques bugs.
  ...
  */
  ...
  getAdjFeminine_fr_FR(adjective: string): string {
    ...
    */
    ...
    const exceptions = {
      ...
      'vengeur': 'vengeresse',
      // Certains adjectifs en -teur font leur féminin en -trice : TODO COMPLETER http://la-conjugaison.nouvelobs.com/regles/grammaire/les-adjectifs-en-teur-206.php
    ...
    if (exceptions[adjective]!=null) return exceptions[adjective];
  ...
  getAdjPlural_fr_FR(adjective: string): string {
