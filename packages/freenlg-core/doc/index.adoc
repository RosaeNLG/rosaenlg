...
Supported languages: some English (via http://compromise.cool/[compromise lib]) and some French (via custom resources).


Status: good enough to write real life NLG applications. Easy to complete with missing verbs etc.
...
console.log( pug.renderFile('testNLG.pug', {
...
....
include node_modules/freenlg/mixins/main.pug
...
* `mixins/main.pug` must be included in one of your template. It gives access to FreeNLG's mixins like `foreach`.
* Pug's parameters *must* have a `util` property that point to a `NlgLib` object.


The `NlgLib` constructor accepts a parameter object with:
...
You can also build a new `NlgLib` from an existing one: `newOne = new NlgLib(oldOne)`. *It is useful as it prevents the reload of the linguistic resources, which can take some time.*
...
*FreeNLG templates are basically pug templates where you use FreeNLG mixins to complete the standard pug syntax.*
...
To do it for real, see <<ref_filter.adoc, Filter mixins and functions>>.
...
FreeNLG uses random numbers to choose synonymic alternatives, to choose synonyms and anaphoras, via the mixins `syn` and `syno_sentences`, and the function `syn_fct`: see <<synonyms>>.
...
* When FreeNLG is initialiazed (via the `NlgLib` constructor), it creates an array of random numbers for its own use
...
The `NlgLib` object accepts a param object. One key can be `forceRandomSeed`. If set, FreeNLG will use this random seed. If not set, it will just randomly choose one.
...
. When you want to make tests with the same synonyms: `new NlgLib({forceRandomSeed: xxx})`
...
| Language for templates | based on Pug, the leading node.js templating engine | proprietary language (but well designed for NLG)
| Language for data prep | javascript | proprietary language
| Ecosystem | the whole node.js ecosystem | incorporated rule engine + very few libraries provided by Yseop
| Connectivity during execution | can connect to anything | impossible
| Ease of authoring for technical users | quite good | not so good
...
| Ease of integration | in any node.js app or via Web Service ; can even run in a browser | via Web Service only
| Continuous integration ability | as any other node.js app | difficult to achieve
| Speed of compilation | very fast (javascript generation) | slow
| Speed of generation | very fast: 1 text < 10 ms | not so fast: 1 text takes 200-300 ms
...
| Functional coverage | emergent | broad (too broad?)
| Reliability | under development | very reliable, years of debug
|=====================================================================
