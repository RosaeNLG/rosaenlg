...
** if the forced alternative is empty, it will not trigger it
...
=== Changed
* `synz 'sequence'` is not valid no more: use `synz {mode:'sequence'}`
* changed the random algorithm: it is still random (at least I hope so) but does not provided exactly the same numbers as before; you shall update your regression tests.
