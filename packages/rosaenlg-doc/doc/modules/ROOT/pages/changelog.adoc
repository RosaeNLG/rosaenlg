...
MUST BE A MAJOR VERSION as there are breaking changes
...
* *breaking change* on `ref` and `refexpr`: they must be reference to mixins (and no more strings), and they must be referenced after being declared
...
* *breaking change* on assembly separators (`last_separator`, `begin_with_general`, etc.): when referencing a mixin, put the mixin as a reference (*no more as a string*), and the mixin must have been defined before

* *breaking change*: mixins are now functions, which a name; you cannot have mixins and objects having the same name, e.g.
....
mixin phone
  | bla bla
- var phone = {};
