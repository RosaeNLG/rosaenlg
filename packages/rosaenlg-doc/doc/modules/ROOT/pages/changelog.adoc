...
* in `value` mixin, ability to generate a possesive determiner, using `personOwner: 1`, e.g. to generate _ma maison_, _ta maison_ in French. The `personOwner` parameter is also available in the `sentence` mixin subject and object groups.
